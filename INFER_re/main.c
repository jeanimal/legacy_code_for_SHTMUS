...
*/
	printf("time = %ld\n", time(0));
	srandom(time(0));
/*  For testing random number generation, since I've had to switch
*   random generators a lot.  Are they half 1's and 0's?  New sequences? */
	printf("sample random nums\n");
	printf("%ld %ld %ld %ld %ld\n",
	New_Rand(2), New_Rand(2), New_Rand(2), New_Rand(2), New_Rand(2));
	printf("%ld %ld %ld %ld %ld\n",
	New_Rand(2), New_Rand(2), New_Rand(2), New_Rand(2), New_Rand(2));
	...
	*/
    ...
    if(useful.cue_reverse && (sim_info.Algchoice != 6))
        {
          if (FITVALIDATE) {
                Check_For_Reverse(alg_var.cue_validity_array, modelling_world_array, useful);
          } else {
                Check_For_Reverse(alg_var.cue_validity_array, world_array, useful);
          }
    	  printf("just after cue_reverse , here is world:");
	 if (FITVALIDATE)
   	  Print_World_Array_Struct(modelling_world_array, useful.num_cities, 
            useful.num_cues);
	 else
	...
	for (Curr_rep = 0; Curr_rep < sim_info.Repchoice; Curr_rep++)
		...
		if(CROSSVALIDATE)
		{
			...
			if(useful.cue_reverse && (sim_info.Algchoice != 6)) {
			  ...
			  if(DEBUG) {
		          	printf("just after cue_reverse , here is world:");
         		  	if (FITVALIDATE)
          				Print_World_Array_Struct(modelling_world_array, 
					 useful.num_cities, useful.num_cues);
         		  	else
          				Print_World_Array_Struct(world_array, 
					 useful.num_cities, useful.num_cues);
			 }
			}
            ...
            {	
		if (!FITVALIDATE) {
			printf("Subject knowledge array, reversed cues:\n");
			Print_World_Array_Struct(ss_knowledge_array,
             	 	 useful.num_cities, useful.num_cues);
		} else {
			printf("FITVALIDATE: subject knows all; reversed cues:\n");
			Print_World_Array_Struct(modelling_world_array,
				useful.num_cities, useful.num_cues);
		}
		...
		if (!FITVALIDATE) {
                 ...
                 Curr_city, score_matrix_ptr, useful, alg_var);
           		...
           		(*algpoint)(modelling_world_array, modelling_world_array,
                 question_array[Curr_quest].obj1,
                 question_array[Curr_quest].obj2, Curr_per_index,
                 Curr_city, score_matrix_ptr, useful, alg_var);
		}
