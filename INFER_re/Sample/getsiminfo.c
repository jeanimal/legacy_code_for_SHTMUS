...
if (SAMPLING) {
        ...
        if (DEBUG)
                printf("sampling: Samplingtype %ld, SampleN %lf\n",
                 samplingtype, sampleN);
        if (samplingtype == 0)
                sample_size= sampleN;
        else if (samplingtype == 1)
                sample_size= sampleN * useful.num_cities;
        if (sample_size < 1 || sample_size > useful.num_cities) {
                printf("error: sample size of %lf\n", sample_size);
                exit(1);
        }

	AStruct->TempSampleSize= sample_size;
}
