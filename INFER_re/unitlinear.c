...
long guess=0;
	...
	for (cue_cntr=0; cue_cntr < useful.num_cues+1; cue_cntr++) {
		cue_val1= ss_knowledge_array[city1].cue[cue_cntr];
		cue_val2= ss_knowledge_array[city2].cue[cue_cntr];
		if (REAL_GREATER(cue_val1, cue_val2))
			weight1++;
		else if (REAL_GREATER(cue_val2, cue_val1))
			weight2++;
	}
