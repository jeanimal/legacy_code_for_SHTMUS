...
/* When you used a trimmed version of unit linear,
 *  (don't use cues with too-low validity),
 *   call algorithm in case 4 ULinT rather than ULin */
case 4: strcpy(AStruct->Algstr,"ULin.");break;
...
void Make_Output_File_Names(struct sim_info_struct *AStruct)
{
	...
	if (!CROSSVALIDATE) {
		printf("cannot turn on FITVALIDATE without CROSSVALIDATE\n");
		printf("fatal error, exiting\n");
		exit(1);
	}
        strcat(AStruct->Correctfilename, ".FV");
        strcat(AStruct->Datafilename, ".FV");
        strcat(AStruct->Lookupfilename, ".FV");
        strcat(AStruct->Guessfilename, ".FV");
    }
