 ...
 *
 * file: unittally.c
 * purpose: after data structures filled, implements the unit tally inference heuristic
 */
...
long
Unit_Tally(struct world_array_struct *ss_knowledge_array,
...
long guess=0;
	...
	for (cue_cntr=0; cue_cntr < useful.num_cues+1; cue_cntr++) {
 	...
 	if ( REAL_EQUAL(tally2,tally1) ) {
        	...
        	if(DEBUG)printf("______________unit tally GUESSED\n");
