...
/* UPDATED March 1998, by Jean Czerlinski */
/*   to take cues in random order */
/* NOT YET DEBUGGED!! */
...
/* CHANGE : copy in cues IN A RANDOM ORDER 
 * This guarantees that when a later cue is dropped because of collinearity
 * with earlier ones, it is not always the same cue(s) that is dropped.
 */
    Draw_Wo_Replacement(cueorder, (long) useful.num_cues);

    for (case_cntr=0; case_cntr<useful.num_cities; case_cntr++)
	for (cue_cntr=1; cue_cntr<useful.num_cues+1; cue_cntr++)
	    MAT(mat, case_cntr, cue_cntr)= 
		 world[case_cntr].cue[1+cueorder[cue_cntr]];
			/* 1+ because  cueorder runs from 0 to
			 * numcues-1 rather than 1 to numcues */

/* CHANGE continues below.  Wherever cues are stepped through,
 * make sure they are stepped through in order of cueorder
 */	
...
for (cue_cntr=0; cue_cntr<useful.num_cues+1; cue_cntr++)
	VEC(Index, cue_cntr)= cueorder[cue_cntr];
*/

    VEC(Index, 0)= 0;
    cues_in_model= 1;
    for (cue_cntr=1; cue_cntr<useful.num_cues+1; cue_cntr++)
        if ( !No_Variation(world, cueorder[cue_cntr], useful.num_cities)
        && !Collinear_With_Earlier(world, cueorder[cue_cntr], useful.num_cities,
                                      Index, cues_in_model)   )
        {
            VEC(Index, cues_in_model)= 1+cueorder[cue_cntr];
            cues_in_model++;
        }

	numTerms= cues_in_model-1;
